...
class CinnamonChip : public SubComponent {
public:
    CinnamonChip(ComponentId_t id, Params &params, CinnamonAccelerator *accelerator, CinnamonNetwork *network, uint32_t chipID);
    ~CinnamonChip();
    ...
    SST_ELI_REGISTER_SUBCOMPONENT_API(SST::Cinnamon::CinnamonChip, CinnamonAccelerator *, CinnamonNetwork *, uint32_t)
    ...
    SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS(
        ...
        {"reader", "Trace Reader to use to", "SST::CinnamonAccelerator::TraceReader"})

private:
    CinnamonChip();                       // Serialization only
    CinnamonChip(const CinnamonChip &);   // Do not impl.
    void operator=(const CinnamonChip &); // Do not impl.
    ...
    friend class CinnamonAccelerator;
