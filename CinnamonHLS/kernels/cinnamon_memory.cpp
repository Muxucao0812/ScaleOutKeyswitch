...
void cinnamon_memory(const std::uint64_t *instructions, std::uint64_t *control_words,
                     std::uint64_t *payload_words, std::uint64_t *outputs,
                     std::uint32_t instruction_count, std::uint32_t control_count,
                     std::uint32_t payload_count, std::uint32_t output_count,
...
#pragma HLS INTERFACE m_axi port = control_words offset = slave bundle = gmem1 depth = 65536 max_widen_bitwidth = 512 num_read_outstanding = 32 max_read_burst_length = 64
#pragma HLS INTERFACE m_axi port = payload_words offset = slave bundle = gmem2 depth = 524288 max_widen_bitwidth = 512 num_read_outstanding = 32 max_read_burst_length = 64
#pragma HLS INTERFACE m_axi port = outputs offset = slave bundle = gmem3 depth = 16384 max_widen_bitwidth = 512 num_write_outstanding = 32 max_write_burst_length = 64
...
#pragma HLS INTERFACE s_axilite port = return bundle = control
  ...
  cinnamon_hls_kernel::execute_memory_module(
